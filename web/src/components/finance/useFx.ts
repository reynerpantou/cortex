...
// One in-flight/settled lookup per currency+day for the whole session, so
// many grid rows in the same currency share a single request.
const cache = new Map<string, Promise<FxQuote | null>>();

export function lookupFx(currency: string, date: string): Promise<FxQuote | null> {
  const key = `${currency}|${date}`;
  let p = cache.get(key);
  if (!p) {
    p = financeApi.fx(currency, date).catch(() => null);
    cache.set(key, p);
...
export function useFx(currency: string, date: string, base: string): FxState {
  ...
  useEffect(() => {
    ...
    void lookupFx(currency, date).then((quote) => {
